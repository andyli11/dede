...
def generate_job(
    ...
    job_type_to_scale_factor_choices=None,
):
    """Generates a new job.
    ...
    if always_generate_scale_factor:
        ...
        scale_factor = scale_factor_generator_func(
            scale_factor_rng, scale_factor_choices
        )
    else:
...
def read_per_instance_type_spot_prices_json(directory):
    per_instance_type_spot_prices = {}
    per_instance_type_spot_prices["aws"] = read_per_instance_type_spot_prices_aws(
        os.path.join(directory, "aws/logs")
    )
    per_instance_type_spot_prices["azure"] = read_per_instance_type_spot_prices_azure(
        os.path.join(directory, "azure/logs")
    )
    per_instance_type_spot_prices["gcp"] = {"v100": 0.74, "p100": 0.43, "k80": 0.135}
    return per_instance_type_spot_prices
...
def get_latest_price_for_worker_type_aws(
    ...
    availability_zones = [
        x["AvailabilityZone"] for x in per_instance_type_spot_prices[instance_type]
    ]
    latest_prices = []
    for availability_zone in set(availability_zones):
        per_instance_type_spot_prices[instance_type].sort(
            key=lambda x: datetime.strptime(x["Timestamp"], "%Y-%m-%dT%H:%M:%S.000Z")
        )
        latest_price = None
        for x in per_instance_type_spot_prices[instance_type]:
...
def get_latest_price_for_worker_type_azure(
    ...
    for zone in per_instance_type_spot_prices[instance_type]:
        per_instance_type_spot_prices[instance_type][zone].sort(key=lambda x: x[0])
        ...
        for worker_type in raw_throughputs:
            ...
            for job_type in raw_throughputs[worker_type]:
                ...
                for other_job_type in raw_throughputs[worker_type][job_type]:
                    ...
                    parsed_throughputs[worker_type][key][other_key] = raw_throughputs[
                        worker_type
            ...
            for job_type_i in range(raw_throughputs.shape[1]):
                for scale_factor_i in range(raw_throughputs.shape[2]):
                    ...
                    parsed_throughputs[f"worker_{worker_type_i}"][key] = {
                        "null": raw_throughputs[
                            worker_type_i, job_type_i, scale_factor_i
                        ]
                    }
...
def get_policy(policy_name, solver=None, seed=None):
    if policy_name == "max_min_fairness_perf":
        ...
        policy = MaxProportionalFairness(
            solver=solver if solver is not None else cp.SCS
        )
