...
class SubproblemsWrap():
    ...
    def update_parameters(self, scale_factors, throughputs, is_valid_idx_d):
        ...
        for prob, throughput, is_valid in zip(self.probs_d, throughputs.T, is_valid_idx_d):
            # Replace NaN and inf values with zeros to ensure real values for CVXPY
            throughput_clean = np.where(
                np.isfinite(throughput), throughput, 0.0
            )
            prob.throughput.value = throughput_clean
